...
function init2(_) {
  return [new Model2(new None()), read_localstorage2("message")];
}
...
function update(model, msg) {
  if (msg instanceof UserUpdatedMessage) {
    let input4 = msg[0];
    return [new Model2(new Some(input4)), write_localstorage2("message", input4)];
  } else if (msg instanceof CacheUpdatedMessage && msg[0].isOk()) {
    let message = msg[0][0];
    return [new Model2(new Some(message)), none()];
  } else {
    return [model, none()];
  }
}
function view(model) {
  let styles = toList([["width", "100vw"], ["height", "100vh"]]);
  let message = unwrap(model.message, "");
  return centre2(
    toList([style(styles)]),
    field3(
      toList([]),
      ...
      toList([text("Type a message and refresh the page")])
    )
  );
}
function main() {
