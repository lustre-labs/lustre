...
function try_get_field(value2, field4, or_else) {
  try {
    return field4 in value2 ? new Ok(new Some(value2[field4])) : or_else();
  } catch {
    return or_else();
  }
}
...
function property(name, value2) {
  return new Attribute(name, from(value2), true);
}
...
var Model = class extends CustomType {
  constructor(value2, length3, max2) {
    super();
    this.value = value2;
    this.length = length3;
    this.max = max2;
...
function view(model) {
    ...
    return try$(
      ...
      (target) => {
        return try$(
          ...
          (value2) => {
            let loud = uppercase2(value2);
            return new Ok(new UserUpdatedMessage(loud));
          }
        );
      }
    );
  ...
  return centre2(
    ...
    aside2(
      ...
      field3(
        toList([]),
        toList([text("Write a LOUD message:")]),
        input3(
          toList([
            value(model.value),
            on2("input", make_it_loud)
          ])
        ),
