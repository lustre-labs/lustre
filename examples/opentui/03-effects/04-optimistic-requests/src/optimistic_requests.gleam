...
type Model {
  Model(todos: Optimistic(List(Todo)), loading: Bool, focus_index: Int)
}
...
fn init(_) -> #(Model, Effect(Msg)) {
  #(
    Model(todos: optimist.from([]), loading: True, focus_index: 0),
    effect.batch([
...
fn update(model: Model, msg: Msg) -> #(Model, Effect(Msg)) {
  case msg {
    UserClickedComplete(id, completed) -> #(
      ...
      Model(
        ..model,
        todos: optimist.update(model.todos, {
    ...
    ApiUpdatedTodo(response) -> #(
      ...
      // optimistic update. If `response` is an `Error` then optimist will
      // automatically revert the optimistic update.
      Model(
        ..model,
        todos: optimist.try(model.todos, response, fn(items, id) {
    ...
    ApiReturnedTodos(Ok(todos)) -> #(
      Model(..model, todos: optimist.from(todos), loading: False),
      case todos {
        [] -> effect.none()
    ...
    KeyPressed(key_event) -> {
      let items = optimist.unwrap(model.todos)
      case key_event.key {
    ...
    [
      tui.box(
        [
          ...
          attribute.padding(1),
          attribute.gap(0),
          attribute.title(" Todo List (Optimistic) "),
