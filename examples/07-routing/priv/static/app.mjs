...
function update(model, msg) {
  if (msg instanceof OnRouteChange) {
    ...
    return [
      (() => {
        let _record = model;
        return new Model2(route, _record.guests, _record.new_guest_name);
      })(),
      none()
    ];
    ...
    return [
      (() => {
        let _record = model;
        return new Model2(_record.current_route, _record.guests, name);
      })(),
      none()
    ];
  } else {
    ...
    return [
      (() => {
        let _record = model;
        return new Model2(
          _record.current_route,
          append(model.guests, toList([guest])),
          ""
        );
      })(),
      none()
...
function view_body(children2) {
  return centre2(toList([mt_xl()]), stack2(toList([]), children2));
}
