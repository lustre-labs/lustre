...
we created a benchmark app to compare Lustre's performance against v4 and popular
JavaScript frameworks. Let's see how v5 stacks up against v4:
...
A lot of Lustre's positioning is around making it easy for backend developers to
build more-interactive frontend experiences. A common question from these folks
is "how do I handle forms?" and typically the response has been that you shouldn't
need forms now that you control input state with your `Model` and handle effects
like HTTP requests in your `update` function.
