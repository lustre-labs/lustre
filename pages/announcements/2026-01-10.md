...
three main points to take note of when considering `element.memo`:
  ...
  1. Values in the dependency list are checked for reference equality _only_. This
     ...
     type every render, even if the values contained haven't changed, the
...
run on) is its fault tolerance achieved through a tree of _supervisors_: special
...
participate in supervision due to Lustre's constraints as a library capable of
...
always welcome! And finally, internally Lustre has begun migrating its JavaScript
