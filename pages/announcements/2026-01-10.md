...
three main points to take note of when considering `element.memo`:
  ...
  1. Values in the dependency list are checked for reference equality _only_. This
     ...
     type ever render, even if the values contained don't changed the _reference_
     to the underlying JavaScript object does change, invalidating the memo cache.
