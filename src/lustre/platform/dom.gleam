...
/// ```
///
pub fn serializer() -> Serializer(message) {
...
/// ```
///
pub fn to_serializer(config: SerializerConfig) -> Serializer(message) {
  Serializer(
...
/// ```
///
pub fn serialize(config: SerializerConfig, node: Element(message)) -> String {
  node
...
pub fn to_document_string_tree(el: Element(message)) -> StringTree {
  ...
  |> string_tree.append_tree(to_string_tree(wrap_document(el), ""))
}
...
fn get_document_type(el: Element(message)) -> DocumentType {
  case el {
    Element(tag: "html", ..) | RawContainer(tag: "html", ..) -> Html
    Element(tag: "head", ..) | RawContainer(tag: "head", ..) -> HeadOnly
    Element(tag: "body", ..) | RawContainer(tag: "body", ..) -> BodyOnly
